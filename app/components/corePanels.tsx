import type { Assignment, ModuleDescriptor, PlannerTask, Priority, ViewKey } from "../domain";
...
export function AppNavigation(props: AppNavigationProps) {
  const { modules, activeView, onNavigate } = props;
  const primaryModules = modules.slice(0, 5);
  const secondaryModules = modules.slice(5);
  const activeInMore = secondaryModules.some((module) => module.key === activeView);
    ...
    <nav className="app-navigation" aria-label="Main navigation">
      <div className="app-navigation-primary">
        {primaryModules.map((module) => (
          <button
            key={module.key}
            className={activeView === module.key ? "active" : ""}
      ...
      {secondaryModules.length ? (
        <details className={`app-navigation-more ${activeInMore ? "active" : ""}`}>
          <summary aria-label="Open more sections">More</summary>
...
export function AuthPanel(props: AuthPanelProps) {
  ...
  return (
    <article className="panel auth-panel">
      ...
      {authMsg ? <div className="status" role="status" aria-live="polite">{authMsg}</div> : null}
    </article>
  );
}
...
export function AssignmentsPanel(props: AssignmentsPanelProps) {
  ...
  return (
    <article className="panel">
      ...
      ) : <p className="empty-state compact-line">Your assignment list is clear. Add the next deadline you want to protect.</p>}
    </article>
  );
}
...
export function PlannerPanel(props: PlannerPanelProps) {
  ...
  return (
    <article className="panel">
      ...
      ) : <p className="empty-state compact-line">Capture one small next step to turn today into momentum.</p>}
    </article>
  );
}
